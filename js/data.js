...
OMT.chapters = [
  ...
  },
  {
    "id": "data",
    "num": "2",
    "part": "Data, Adaptation and Evaluation",
    "file": "chapters/data-and-datasets.html",
    "title": "Where Model Data Comes From",
    "minutes": 12,
    "blurb": "Inspect a library dataset: distinguish source documents, demonstrations and test cases, then investigate selection, provenance and coverage.",
    "keywords": "Distinguish data used in context, training and evaluation. Explain how selection and provenance affect a dataset’s use. Design a data split and identify missing coverage.  The library has inspected Cedar’s release and found a possible route to running its own assistant. Now someone asks, “Can we just give it our documents?” The librarian opens a folder containing a current notice, an old notice, a visitor’s message and an answer written by another model. All are text. They are not interchangeable. \n We will continue with  Alder, our fictional library application , using Cedar as its underlying model. The pilot’s job is to answer questions from public notices and identify the supporting notice. It should say when the supplied information is insufficient. It will not access visitor accounts or reserve books. Every library record and decision in this chapter is invented for teaching. \n  What you will be able to do   Distinguish documents supplied at answer time from examples used for training and evaluation.  Explain how selection, cleaning and provenance affect what a dataset can support.  Propose a defensible separation of development and test material, and identify missing coverage.   \n  Before you start:  does placing a notice in a request change the model’s weights? Does permission to download a model also settle permission to use a visitor’s message? If either distinction is unclear, revisit  context and training  and  component permissions . In the ordinary inference route, weights stay fixed; the visitor message requires its own assessment. \n\n 1. Start with a record and a purpose \n A  dataset  is a collection of records assembled for a purpose. A record is one item with fields that help us interpret it. It might be a whole document, a paragraph, a question paired with an answer, or two answers with a preference between them. A large collection of texts is also called a  corpus . These names describe organisation, not quality. \n  A fictional record before it becomes training data   Record  N-17  Document  Sunday service notice, revision 2  Text  The library is open on Sundays from 12:00 to 16:00. Books can be returned at the desk while the library is open.  Origin and check  Written by library staff; checked against the current approved notice.  Permitted scope in this exercise  The library has confirmed use of this staff-owned public notice for the pilot.   \n The text tells us something about Sunday service. The other fields tell us which text we have and why the team thinks it belongs here. Those fields are  metadata : ",
    "quizId": "data-v1"
  },
  {
    "id": "adaptation",
    "num": "3",
    "part": "Data, Adaptation and Evaluation",
    "file": "chapters/model-adaptation.html",
    "title": "How Models Are Adapted",
    "minutes": 12,
    "blurb": "Follow examples, preferences and teacher outputs into model changes, then design a comparison that can show what helped.",
    "keywords": "Choose an intervention at the context, application or parameter level. Trace demonstrations, preferences and teacher–student signals. Design a comparison that addresses alternative explanations.  Alder now receives the library’s current notices, but its answers are still troublesome. Sometimes it ignores the notice. Sometimes it answers correctly and then adds an invented detail. Sometimes it gives a page-long reply to a simple question. “Train it on our data” sounds like one remedy. These failures may need different changes. \n In the previous chapter, we separated source documents, training examples and evaluation cases. Here we follow what those records actually do. Alder remains the fictional application, Cedar its underlying model. Our aim is to explain an intervention and the evidence needed to assess it—not assume that more training is always the next step. \n  What you will be able to do   Choose between a context change, an application change and a weight update for a stated problem.  Trace information through demonstrations, preferences, rewards and teacher–student training.  Propose a controlled comparison that distinguishes an intervention’s effect from other changes.   \n  Before you start:  a model writes a practice answer, a librarian checks it, and the team uses it in training. Which object supplies the update, and which data should remain outside that process? The checked example supplies the training signal; reserved evaluation cases should not become practice answers. Revisit  data roles  and  data separation  if needed. \n\n 1. Diagnose the failure before choosing a method \n  Adaptation  broadly means changing how a model or system behaves for a purpose. Authors use the word at different levels. In this chapter, we specify whether an intervention changes the request, the surrounding application or trainable parameters. That is more informative than saying the system “learned.” \n Imagine the application retrieves the obsolete Sunday notice. The first candidate fix is the retrieval collection or its selection logic. Retraining Cedar while still supplying the wrong notice would not directly repair that fault. If the notice is correct but Alder overlooks it, the team could first test clearer instructions and examples in the prompt. If the team wants a repeated response style across many tasks, training on demonstrations becomes another candidate. These are hypotheses to compare, not guaranteed cures. \n   Locate what changes    Intervention  What changes?  A useful check      Prompt with a cited example  Context supplied on each request  D",
    "quizId": "adaptation-v1"
  },
  {
    "id": "evaluation",
    "num": "4",
    "part": "Data, Adaptation and Evaluation",
    "file": "chapters/model-evaluation.html",
    "title": "How to Judge a Model",
    "minutes": 12,
    "blurb": "Score real-shaped fictional answers, reproduce a weighted comparison and defend a conditional next step under uncertainty.",
    "keywords": "Specify evaluation conditions and score outputs under a rule. Calculate and interpret a task-weighted comparison. Recommend a next step with counterarguments and uncertainty.  The library has prepared its notices and designed a comparison. Someone returns with a chart: Alder scored 60%; Birch scored 70%. “Choose Birch,” they say. But what was counted, what did the systems receive, and which mistakes matter to the library? A result becomes useful when we can answer those questions. \n This chapter brings the section together. We will score answers, calculate a small comparison and write a conditional next step. All library outputs, system names and numbers below are fictional. Alder and Birch name  complete evaluated systems , not measured commercial models. The real research case explains an evaluation approach, not a current ranking. \n  What you will be able to do   Specify an evaluation’s task, inputs, conditions and scoring rule.  Calculate and interpret a weighted comparison without hiding per-task results.  Explain uncertainty and recommend a next step that the available evidence can support.   \n  Before you start:  does an improvement after retraining show that the new training data caused it? Only if the comparison addresses other relevant changes. Revisit  baselines and controls . Also recall why a  reserved test set  should not guide repeated development choices. \n\n 1. Write down what the result is meant to establish \n An  evaluation  is a procedure for examining performance against specified criteria. A  metric  turns some aspect of that performance into a measurement. A  benchmark  supplies a defined collection of tasks and an evaluation procedure for comparison. These are tools for answering questions; they do not decide which question matters. \n The library’s question is whether a public-notice assistant can give supported answers under a bounded pilot. A researcher might instead ask whether a training method improves a particular capability. A procurement team might compare complete services, including speed and operating work. The same score need not answer all three questions. Stanford’s  CS336 evaluation lecture  distinguishes the inputs, model interaction, scoring and interpretation, and asks whether the object is a model or a larger system. \n Write a short  evaluation contract  before viewing the results. This is our name for the record of the question and conditions, not a legal agreement: \n   Task and coverage:  which visitor questions, documents, languages and exceptions are included?   Object:  which model/c",
    "quizId": "evaluation-v1"
  }
];
...
OMT.glossary = {
  ...
  "dataset": [
    "Dataset",
    "A collection of records assembled for a purpose, such as training, development or evaluation. Its name and size do not establish quality."
  ],
  "fine-tuning": [
    "Fine-tuning",
  ...
  "provenance": [
    "Provenance",
    "Information about a record’s origin, version and transformations, including generation and review where relevant."
  ],
  "data-leakage": [
    "Data leakage",
    "Information crossing a boundary intended to keep an evaluation meaningful, such as derivative training examples entering a supposedly independent test."
  ],
  "development-set": [
    "Development / validation set",
    "Cases used to choose or improve prompts, settings, models or other design decisions, distinct from a reserved final test."
  ],
  "test-set": [
    "Test set",
    "Cases reserved to evaluate the selected approach. Results used repeatedly to change the design become part of development."
  ],
  "baseline": [
    "Baseline",
    "The arrangement used as a reference for a comparison, such as an existing system or a simple alternative."
  ],
  "weighted-mean": [
    "Weighted mean",
    "A sum of category values multiplied by their chosen shares of the total. State what the weights represent."
  ],
  "confounder": [
    "Confounder",
    "An additional difference that makes attributing an observed result to a particular intervention ambiguous."
  ],
  "generalisation": [
    "Generalisation",
    "Performance on new relevant cases beyond the examples used to develop a model or system."
  ],
  "metric": [
    "Metric",
    "A defined measurement of an aspect of performance. It is useful only in relation to a task, conditions and decision."
  ]
};
...
OMT.charts = [
  {
    "id": "task-scores",
    "kind": "fictional",
    "target": "chapters/model-evaluation.html",
    "title": "Fictional correct answers out of ten per task",
    "provenance": "Authored teaching data; Alder and Birch are fictional complete systems, not real model measurements. Reused from the archived teaching example; no empirical performance is claimed.",
    "transformation": "Equal mean = 50 × (manual successes + arithmetic successes) / 10. Weighted percentage = 100 × [w × manual successes / 10 + (1 − w) × arithmetic successes / 10], where w is the chosen manual share.",
    "unit": "percent successful answers under the stated task rules",
